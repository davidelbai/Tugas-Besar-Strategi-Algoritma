...
|----|----------|------|-----------------|
| 1 | **NgaBoot** | **Utama** | Aggressive Pursuit & Dynamic Firepower |
...
```powershell
Start-Process powershell -ArgumentList "cd bots\NgaBoot; dotnet run"
...
├── README.md                 <- file ini
├── NgaBoot/
│   ├── NgaBoot.cs            <- kode utama bot
│   ├── NgaBoot.csproj        <- konfigurasi project .NET
│   ├── NgaBoot.json          <- metadata bot (nama, versi, author)
│   ├── NgaBoot.cmd           <- script jalankan (Windows)
│   └── NgaBoot.sh            <- script jalankan (Linux/Mac)
├── SniperBot/
│   ├── SniperBot.cs
│   ├── SniperBot.csproj
│   ├── SniperBot.json
